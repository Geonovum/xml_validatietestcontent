...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:function name="foo:generateOWId">
        ...
        <xsl:choose>
            <xsl:when test="contains($owId, 'FOUT')">
                <xsl:value-of select="$owId"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="$maxLength > 13">
                        <xsl:variable name="dateString" select="concat(tokenize($owId, '\.')[4], $alreadyRetrievedDateTime)"/>
                        <xsl:value-of select="concat(tokenize($owId, '\.')[1], '.', tokenize($owId, '\.')[2], '.', tokenize($owId, '\.')[3], '.', $dateString)"/>
                    </xsl:when>
                    <xsl:when test="$maxLength > 0 and $maxLength &lt; 14">
                        <xsl:variable name="dateString" select="concat(tokenize($owId, '\.')[4], substring($alreadyRetrievedDateTime, 14 - $maxLength))"/>
                        <xsl:value-of select="concat(tokenize($owId, '\.')[1], '.', tokenize($owId, '\.')[2], '.', tokenize($owId, '\.')[3], '.', $dateString)"/>
                    </xsl:when>
                    <xsl:otherwise>
