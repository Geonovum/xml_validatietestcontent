            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="$maxLength > 13">
                        <xsl:variable name="dateString" select="concat(tokenize($oldId, '\.')[4], $alreadyRetrievedDateTime)"/>
                    ...
                    <xsl:when test="$maxLength > 0 and $maxLength &lt; 14">
                        <xsl:variable name="dateString" select="concat(tokenize($oldId, '\.')[4], substring($alreadyRetrievedDateTime, 14 - $maxLength))"/>
                        <xsl:value-of select="concat(tokenize($oldId, '\.')[1], '.', tokenize($oldId, '\.')[2], '.', tokenize($oldId, '\.')[3], '.', $dateString)"/>
                    </xsl:when>
                    <xsl:otherwise>
