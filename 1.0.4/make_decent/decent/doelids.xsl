...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    xmlns:manifest-ow="http://www.geostandaarden.nl/bestanden-ow/manifest-ow" xmlns:data="https://standaarden.overheid.nl/stop/imop/data/" xmlns:foo="http://whatever">
    ...
    <xsl:template match="manifest-ow:DoelID[text() = $oldDoelId]">
        <xsl:element name="manifest-ow:DoelID">
            <xsl:choose>
                <xsl:when test="contains($doelId, 'TPOD2000')">
                    <xsl:value-of select="''"/>
                </xsl:when>
                <xsl:otherwise>
