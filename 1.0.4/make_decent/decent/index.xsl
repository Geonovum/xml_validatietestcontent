...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
        ...
        <xsl:for-each select="tokenize($file.list, ';')">
            ...
            <xsl:if test="document($fullname)/aanlevering:AanleveringBesluit">
                ...
                <xsl:element name="besluit">
                    ...
                    </xsl:element>                    <xsl:element name="oldInstrumentversie">
            ...
            <xsl:if test="document($fullname)//lvbb:publicatieOpdracht or document($fullname)//lvbb:validatieOpdracht">
                <xsl:element name="leveringId">
                    ...
                    <xsl:element name="new">
                        <xsl:value-of select="concat('id-publicatie-', $org.file.dir)"/>
                    </xsl:element>
                </xsl:element>
            </xsl:if>
            <!-- OW-Ids -->
    ...
    <xsl:function name="foo:generateDoelId">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($oldId, 'FOUT')">
                <xsl:value-of select="
                    concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', concat($org.file.dir, 'FOUT'))"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', $org.file.dir)"
                />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="foo:generateFRBRWork">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($oldId, 'FOUT')">
                <xsl:value-of select="
                    concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', concat(tokenize($oldId, '/')[7], '-', $org.file.dir, 'FOUT'))"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', concat(tokenize($oldId, '/')[7], '-', $org.file.dir))"
                />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="foo:generateFRBRExpression">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($oldId, 'FOUT')">
                <xsl:value-of 
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', concat(tokenize($oldId, '/')[7], '-', $org.file.dir, 'FOUT'), '/', tokenize($oldId, '/')[8])"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', concat(tokenize($oldId, '/')[7], '-', $org.file.dir ), '/', tokenize($oldId, '/')[8])"
                />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="foo:generateAKNFRBRWork">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($oldId, 'FOUT')">
                <xsl:value-of 
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', concat($org.file.dir, 'FOUT'))"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', $org.file.dir)"
    ...
    <xsl:function name="foo:generateAKNFRBRExpression">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($oldId, 'FOUT')">
                <xsl:value-of 
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', concat($org.file.dir, 'FOUT'), '/', tokenize($oldId, '/')[8])"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="concat('/', tokenize($oldId, '/')[2], '/', tokenize($oldId, '/')[3], '/', tokenize($oldId, '/')[4], '/', tokenize($oldId, '/')[5], '/', tokenize($oldId, '/')[6], '/', $org.file.dir, '/', tokenize($oldId, '/')[8])"
                />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="foo:generateOWId">
        ...
        <xsl:choose>
            <xsl:when test="contains($oldId, 'FOUT')">
                <xsl:value-of select="concat(tokenize($oldId, '\.')[1], '.', tokenize($oldId, '\.')[2], '.', tokenize($oldId, '\.')[3], '.', $seeder, 'FOUT')"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="concat(tokenize($oldId, '\.')[1], '.', tokenize($oldId, '\.')[2], '.', tokenize($oldId, '\.')[3], '.', $seeder)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
